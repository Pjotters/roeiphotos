...
import { uploadMultiplePhotos } from '@/lib/storage-firebase';
...
type PhotoUploaderProps = {
  ...
  onUploadComplete?: (photoData: any[]) => void;
  onUploadError?: (error: Error) => void;
  maxFiles?: number;
  isPublic?: boolean;
};

export default function PhotoUploaderFirebase({
  eventInfo,
  onUploadComplete,
  onUploadError,
  maxFiles = 10,
  ...
  const handleUpload = async () => {
    ...
    if (!user || !userData) {
      setError('Je moet ingelogd zijn om fotos te uploaden.');
      return;
    }
    ...
    try {
      ...
      // Upload de bestanden naar Firebase Storage
      const result = await uploadMultiplePhotos(
        files, 
        userData.id, // Fotograaf ID
        eventInfo,
        isPublic
      );
      ...
      if (onUploadComplete) {
        onUploadComplete(result.filter((item: any) => item.success).map((item: any) => ({
          url: item.url,
          path: item.path,
